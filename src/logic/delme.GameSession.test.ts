...
import { Problem, ProblemGenerator } from "./Problems/ProblemGenerators";
...
describe("GameSession", () => {
	...
	it("counts a correct answer and advances to the next problem", () => {
		...
		expect(ui.informUser).toHaveBeenCalledWith("Точно така!", "#00c000");
	...
	it("counts an incorrect answer and keeps the same problem", () => {
		...
		expect(ui.showProblem).toHaveBeenCalledTimes(1);
		expect(ui.informUser).toHaveBeenCalledWith("Пробвай пак.", "#ff4020");
