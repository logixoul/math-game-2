...
export class ProblemScheduler {
	...
	postponeProblem(problemToPostpone: Problem): void {
		const existing = this.postponedProblems.find(
			(postponedProblem) =>
				postponedProblem.problem.text === problemToPostpone.text,
		);
		if (existing) {
			existing.turnsRemaining = NUM_POSTPONEMENT_TURNS;
		} else {
			this.postponedProblems.push(
				new ProblemPostponement(problemToPostpone, NUM_POSTPONEMENT_TURNS),
			);
		}
	}
	...
	updateTurnsRemainingCounters() {
		for (const postponedProblem of this.postponedProblems) {
			postponedProblem.turnsRemaining = Math.max(
				0,
				postponedProblem.turnsRemaining - 1,
			);
		}
	}
	...
	*generateProblems(): Generator<Problem, void, unknown> {
		while (true) {
			...
			const dueIndex = this.postponedProblems.findIndex(
			...
			const postponedKeys = new Set(
				this.postponedProblems.map((p) => p.problem.text),
			);
